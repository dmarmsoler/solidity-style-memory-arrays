...
lemma is_Array_minit:
  assumes "Memory.minit cd ba = (l, baa)"
      ...
      and "baa $ a = Some b"
      and "\<exists>as. dlookup y cd = Some (sdata.Array as)"
    shows "mdata.is_Array b"
  using assms
proof -
  have *: "s_data.copy_memory_safe {||} baa l = Some cd"
    using minit_copy[OF assms(1)] by (simp add: s_data.copy_memory_def prefix_id)
  then obtain as where "s_data.copy_memory_safe {||} baa a = Some (sdata.Array as)" using copy_memory_calldata_mlookup_obtains[OF * assms(2)]
    using assms(4) by fastforce
  then show ?thesis using assms(3)
    by (metis assms(2) sdata.distinct(1) copy_memory_safe_def data.mlookup_copy_memory_safe mdata.collapse(1) mdata.exhaust_disc)
qed
...
definition pred_memory where
  "pred_memory i P r s =
    (case (Stack s) $$ i of
      Some (kdata.Memory l) \<Rightarrow> pred_some P (copy_memory_calldata (State.Memory s) l)
    | _ \<Rightarrow> False)"

text \<open>Needs to be used manually\<close>
lemma pred_some_copy_memory:
  assumes "copy_memory_calldata m l = Some cd"
      and "P cd"
    shows "pred_some P (copy_memory_calldata m l)"
...
lemma mlookup_some_minit:
  assumes "Memory.minit cd m0 = (l0, m1)"
      and "\<not>Option.is_none (dlookup xs cd)"
...
proof -
  ...
  moreover from minit_locs_calldata[OF assms(1)] obtain L
    where "locs_safe {||} m1 l0 = Some L"
      and *: "fset L \<inter> loc m0 = {}"
      unfolding s_disj_fs_def pred_some_def Utils.s_union_fs_def pred_some_def unfolding locs_safe_def locs_calldata_def data.locs_def by blast
  then have "l2 |\<in>| L" using s_data.locs_safe_mlookup assms(2) by blast
...
proof -
  from Memory.minit_loc[OF assms(1)] obtain L
    where "locs_calldata m1 l0 = Some L"
      and "loc m1 = loc m0 \<union> fset L"
    unfolding Utils.s_union_fs_def pred_some_def locs_safe_def locs_calldata_def data.locs_def
    by blast
  moreover from mlookup_some[OF assms(1)] assms(2)
  obtain y where "mlookup m1 xs l0 = Some y"
    by force
  ultimately show ?thesis
    by (metis UnCI option.sel s_data.locs_mlookup)
qed
...
lemma mlookup_locations_minit_2:
  assumes "Memory.minit cd m0 = (l0, m1)"
      and "\<not> Option.is_none (dlookup ys cd)"
      and "mlookup m0 xs l1 = Some (the (mlookup m0 xs l1))"
      and "the (mlookup m0 xs l1) \<in> loc m0"
...
proof -
  ...
  moreover obtain L where L_def: "locs_calldata m1 l0 = Some L" and **: "fset L \<inter> loc m0 = {}"
    using minit_locs_calldata[OF assms(1)] unfolding pred_some_def s_disj_fs_def by auto
  ultimately have "L' |\<subseteq>| L" using s_data.locs_locations by blast
...
lemma locs_locs_minit_1:
  assumes "Memory.minit cd m0 = (l0, m1)"
    shows "locs_calldata m1 l0 = Some (the (locs_calldata m1 l0))"
...
lemma locs_locs_minit_2:
  assumes "Memory.minit cd m0 = (l1, m1)"
    and "locs_calldata m0 l = Some (the (locs_calldata m0 l))"
  shows "locs_calldata m1 l = Some (the (locs_calldata m1 l))"
proof -
  from assms(1) have "prefix m0 m1"
    by (metis minit_sprefix snd_eqD sprefix_prefix)
  then show ?thesis
    by (metis assms(2) s_data.locs_prefix)
qed

lemma locs_locs_disj_minit:
  assumes "Memory.minit cd m0 = (l1, m1)"
    and "locs_calldata m0 l0 = Some (the (locs_calldata m0 l0))"
  shows "the (locs_calldata m1 l0) |\<inter>| the (locs_calldata m1 l1) = {||}"
proof -
  from assms(1) have "prefix m0 m1"
    by (metis minit_sprefix snd_eqD sprefix_prefix)
  moreover from assms(2) have "fset (the (locs_calldata m0 l0)) \<subseteq> loc m0" using s_data.locs_subs2 by auto
  ultimately have "fset (the (locs_calldata m1 l0)) \<subseteq> loc m0"
    by (metis assms(2) s_data.locs_prefix)
  then show ?thesis using minit_locs_calldata[OF assms(1)] unfolding s_disj_fs_def pred_some_def
    by auto
qed
...
lemma locs_some_mupdate_1:
  assumes "mvalue_update is1 (l1, v, m0) = Some m1"
      ...
      and "m0 $ l2' = Some v"
      and "check_calldata m0 (the (locs_calldata m0 l1))"
      and "the (locs_calldata m0 l1) |\<inter>| the (locs_calldata m0 l2) = {||}"
      and "locs_calldata m0 l2 = Some (the (locs_calldata m0 l2))"
      and "locs_calldata m0 l1 = Some (the (locs_calldata m0 l1))"
    shows "locs_calldata m1 l1 = Some (the (locs_calldata m1 l1))"
proof -
  from assms(1) obtain l
  ...
  moreover from assms(7) obtain L1 where L1_def: "locs_calldata m0 l1 = Some L1" by simp
  moreover from assms(2,6) obtain L2 where L2_def: "locs_calldata m0 l2' = Some L2" and *: "L2 |\<subseteq>| the (locs_calldata m0 l2)"
    by (metis s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover from L1_def obtain L1' where "locs_calldata m0 l = Some L1'" using l_def
    by (metis s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have "\<forall>l|\<in>|L1 |-| L1'. m1 $ l = m0 $ l"
  proof
    fix l' assume "l' |\<in>|L1 |-| L1'"
    moreover have "l |\<notin>| L1 |-| L1'"
      by (meson \<open>locs_calldata m0 l = Some L1'\<close> fminusD2 s_data.locs_subs)
  ...
  proof
    ...
    moreover have "l |\<in>|L1"
      using L1_def l_def s_data.locs_mlookup by blast
    ultimately have "l \<noteq> l'" using assms(5) L1_def L2_def * by auto
    then show "m1 $ l' = m0 $ l'" unfolding nth_safe_def using ** by (simp)
  qed
  moreover from assms(4) have "check_calldata m0 L1" using L1_def by auto
  moreover have "the (locations m0 is1 l1) |\<inter>| L2 = {||}"
  proof -
    from l_def obtain LL where "locations m0 is1 l1 = Some LL"
      using mlookup_locations_some by blast
    then have "LL |\<subseteq>| L1" using L1_def using s_data.locs_locations by blast
...
lemma locs_some_mupdate_2:
  assumes "mvalue_update xs (l0, v, m0) = Some m1"
    and "the (mlookup m0 xs l0) |\<notin>| the (locs_calldata m0 l)"
    and "locs_calldata m0 l = Some (the (locs_calldata m0 l))"
  shows "locs_calldata m1 l = Some (the (locs_calldata m1 l))"
proof -
  obtain l'
    ...
    and m'_def: "m1 = m0[l':=v]"
  using mvalue_update_obtain[OF assms(1)] by auto
  moreover from assms(2) have "\<forall>l''|\<in>|the (locs_calldata m0 l). m1 $ l'' = m0 $ l''"
    using 0 m'_def unfolding nth_safe_def apply (simp split:if_split_asm)
    by (metis nth_list_update_neq)
  ultimately show ?thesis using s_data.locs_same[of m0 l] by (metis assms(3))
qed

lemma locs_disj_minit:
assumes "Memory.minit z m0 = (laa, m1)"
    and "locs_calldata m0 x1 = Some (the (locs_calldata m0 x1))"
    and "locs_calldata m0 la = Some (the (locs_calldata m0 la))"
    and "the (locs_calldata m0 x1) |\<inter>| the (locs_calldata m0 la) = {||}"
  shows "the (locs_calldata m1 x1) |\<inter>| the (locs_calldata m1 la) = {||}"
proof -
  from assms(1) have "prefix m0 m1"
    by (metis minit_sprefix snd_conv sprefix_prefix)
  moreover from assms(2) obtain L where "locs_calldata m0 x1 = Some L" by blast
  ultimately have "locs_calldata m1 x1 = Some L" using s_data.locs_prefix by auto
  moreover from assms(3) obtain L' where L'_def: "locs_calldata m0 la = Some L'" by blast
  moreover have "locs_calldata m1 la = Some L'" using L'_def `prefix m0 m1` s_data.locs_prefix by auto
  ultimately show ?thesis using assms(4)
    by (simp add: \<open>locs_calldata m0 x1 = Some L\<close>)
qed
...
lemma locs_disj_mupdate:
  assumes "mvalue_update is1 (l1, v, m0) = Some m1"
      ...
      and "locs_calldata m0 la = Some (the (locs_calldata m0 la))"
      and "the (locs_calldata m0 l1) |\<inter>| the (locs_calldata m0 l2) = {||}"
      and "the (mlookup m0 is1 l1) |\<notin>| the (locs_calldata m0 la)"
      and "check_calldata m0 (the (locs_calldata m0 l1))"
      and "the (locs_calldata m0 l1) |\<inter>| the (locs_calldata m0 la) = {||}"
      and "the (locs_calldata m0 l2) |\<inter>| the (locs_calldata m0 la) = {||}"
    shows "the (locs_calldata m1 l1) |\<inter>| the (locs_calldata m1 la) = {||}"
proof -
  from assms(1) obtain l
    where l_def: "mlookup m0 is1 l1 = Some l"
    and *: "l < length m0"
    and **: "m1 = m0[l:=v]" using mvalue_update_obtain by metis
  then have 0: "m1 $ l = m0 $ l2'" by (simp add: assms(3))
  moreover obtain L0 where "locs_calldata m1 l1 = Some L0" using locs_some_mupdate_1[OF assms(1,2,3,9,7,5,4)] by simp
  moreover from assms(4) obtain L1 where L1_def: "locs_calldata m0 l1 = Some L1" by simp
  moreover from assms(5) obtain L2 where L2_def: "locs_calldata m0 l2' = Some L2" and ***: "L2 |\<subseteq>| the (locs_calldata m0 l2)"
    by (metis assms(2) s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have "locs_calldata m0 l = Some (the (locs_calldata m0 l))"
    by (metis L1_def l_def option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have "\<forall>l|\<in>|L1 |-| the (locs_calldata m0 l). m1 $ l = m0 $ l" using ** unfolding nth_safe_def apply (simp split:if_split_asm)
    by (metis Diff_iff calculation(6) nth_list_update_neq s_data.locs_subs)
  moreover have "\<forall>l|\<in>|the (locs_calldata m0 l2). m1 $ l = m0 $ l"
  proof
    fix l' assume "l'|\<in>|the (locs_calldata m0 l2)"
    moreover from l_def have "l|\<in>|the (locs_calldata m0 l1)" using s_data.locs_mlookup[OF L1_def] L1_def by simp
  ...
  moreover from assms(9) have "check_calldata m0 L1" using L1_def by simp
  ultimately have "L0 |\<subseteq>| L1 |\<union>| L2" using locs_update_subs[OF l_def 0, of L1 "the (locs_calldata m0 l)" L2 L0] by blast
  moreover have "L2 |\<subseteq>| the (locs_calldata m0 l2)"
    by (metis \<open>locs_calldata m0 l2' = Some L2\<close> assms(2,5) s_data.locs_def s_data.mlookup_locs_subs)
  moreover have "the (locs_calldata m0 la) = the (locs_calldata m1 la)"
  proof -
    obtain L where L_def: "locs_calldata m0 la = Some L" using assms(6) by simp
    ...
    ultimately have "locs_calldata m1 la = Some L"
      by (metis s_data.locs_same)
    then show ?thesis using L_def by simp
  qed
  ultimately show ?thesis using assms(10,11)
    using \<open>locs_calldata m1 l1 = Some L0\<close> L1_def by auto
qed
...
lemma mlookup_locs_minit:
  assumes "Memory.minit cd m0 = (l0, m1)"
    ...
    shows "the (mlookup m1 is1 l1) |\<notin>| the (locs_calldata m1 l0)"
proof -
  from assms(1) obtain L where L_def: "locs_calldata m1 l0 = Some L"
    using locs_locs_minit_1 by blast
  then have "fset L \<inter> loc m0 = {}"
    by (metis Diff_disjoint Memory.minit_loc assms(1) inf_commute minit_locs_calldata option.sel s_disj_union_fs)
...
lemma mlookup_locs_minit2:
  assumes "Memory.minit cd m0 = (l0, m1)"
      and "mlookup m0 is1 l1 = Some (the (mlookup m0 is1 l1))"
      and "locs_calldata m0 l = Some (the (locs_calldata m0 l))"
      and "the (mlookup m0 is1 l1) |\<notin>| the (locs_calldata m0 l)"
    shows "the (mlookup m1 is1 l1) |\<notin>| the (locs_calldata m1 l)"
proof -
  from assms(1) have "prefix m0 m1"
    by (metis minit_sprefix snd_conv sprefix_prefix)
  then have "the (mlookup m0 is1 l1) = the (mlookup m1 is1 l1)"
    by (metis assms(2) mlookup_prefix_mlookup)
  moreover have "the (locs_calldata m0 l) = the (locs_calldata m1 l)"
    by (metis \<open>prefix m0 m1\<close> assms(3) s_data.locs_prefix)
  ultimately show ?thesis using assms(4) by simp
qed
...
lemma copy_minit:
  assumes "Memory.minit cd m0 = (l0, m1)"
      and "locs_calldata m0 l1 = Some (the (locs_calldata m0 l1))"
      and "copy_memory_calldata m0 l1 = Some cd'"
    shows "copy_memory_calldata m1 l1 = Some cd'"
  using assms
  by (metis minit_sprefix s_data.copy_memory_append snd_conv sprefix_prefix)

lemma copy_mupdate_value:
  assumes "mvalue_update xs (l0, mdata.Value v, m0) = Some m1"
      and "check_calldata m0 (the (locs_calldata m0 l0))"
      and "copy_memory_calldata m0 l0 = Some cd0"
    shows "copy_memory_calldata m1 l0 = Some (the (updateCalldata xs (sdata.Value v) cd0))" 
proof-
  from assms(1) obtain l
    where l_def: "mlookup m0 xs l0 = Some l"
    and *: "l < length m0"
    and **: "m1 = m0[l:=mdata.Value v]" using mvalue_update_obtain by metis
  moreover have 1: "locs_safe {||} m0 l0 = Some (the (locs_safe {||} m0 l0))" using s_data.locs_copy_memory_some
    by (metis assms(3) option.sel s_data.copy_memory_def)
  moreover have 2: "locs_safe {||} m0 l = Some (the (locs_safe {||} m0 l))"
    by (metis "1" l_def option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have 3: "\<forall>l'|\<in>|the (locs_safe {||} m0 l0) |-| (the (locs_safe {||} m0 l)). m1 $ l' = m0 $ l'"
  proof
    fix l' assume "l' |\<in>|the (locs_safe {||} m0 l0) |-| (the (locs_safe {||} m0 l))"
    moreover have "l |\<notin>| the (locs_safe {||} m0 l0) |-| (the (locs_safe {||} m0 l))"
      by (meson "2" fminusD2 s_data.locs_safe_subs)
    ultimately have "l \<noteq> l'" by blast
    then show " m1 $ l' = m0 $ l'" unfolding nth_safe_def using ** by (simp)
  qed
  moreover from assms(2) have 4: "copy_memory_safe {||} m0 l0 = Some (the (copy_memory_safe {||} m0 l0))"
    by (metis assms(3) option.sel s_data.copy_memory_def)
  moreover from assms obtain cd' where 6: "copy_memory_safe {||} m1 l0 = Some cd'"
    by (metis mvalue_update_obtain s_data.copy_memory_calldata_update_value
        s_data.copy_memory_def)
  ultimately show ?thesis using copy_memory_safe_lookup_update_value[OF l_def * ** 1 ]
    by (metis assms(2,3) option.distinct(1) option.exhaust_sel s_data.copy_memory_def s_data.locs_def)
qed
...
lemma copy_mupdate:
  assumes "mvalue_update is1 (l1, v, m0) = Some m1"
      ...
      and "check_calldata m0 (the (locs_calldata m0 l1))"
      and "the (locs_calldata m0 l1) |\<inter>| the (locs_calldata m0 l2) = {||}"
      and "copy_memory_calldata m0 l1 = Some cd1"
      and "copy_memory_calldata m0 l2 = Some cd2"
    shows "copy_memory_calldata m1 l1 = 
      Some (the (dlookup is2 cd2 \<bind> (\<lambda>cd. updateCalldata is1 cd cd1)))"
proof-
  from assms(1) obtain l
    where l_def: "mlookup m0 is1 l1 = Some l"
    and *: "l < length m0"
    and **: "m1 = m0[l:=v]" using mvalue_update_obtain by metis
  then have 0: "m1 $ l = m0 $ l2'" by (simp add: assms(3))
  moreover have 1: "locs_safe {||} m0 l1 = Some (the (locs_safe {||} m0 l1))" using s_data.locs_copy_memory_some
    by (metis assms(6) option.sel s_data.copy_memory_def)
  moreover have 2: "locs_safe {||} m0 l = Some (the (locs_safe {||} m0 l))"
    by (metis "1" l_def option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have 3: "locs_safe {||} m0 l2= Some (the (locs_safe {||} m0 l2))"  using s_data.locs_copy_memory_some
    by (metis assms(7) option.sel s_data.copy_memory_def)
  moreover have 4: "\<forall>l|\<in>|the (locs_safe {||} m0 l1) |-| the (locs_safe {||} m0 l). m1 $ l = m0 $ l"
  proof
    fix l' assume "l' |\<in>|the (locs_safe {||} m0 l1) |-| (the (locs_safe {||} m0 l))"
    moreover have "l |\<notin>| the (locs_safe {||} m0 l1) |-| (the (locs_safe {||} m0 l))"
      by (meson "2" fminusD2 s_data.locs_safe_subs)
    ultimately have "l \<noteq> l'" by blast
    then show "m1 $ l' = m0 $ l'" unfolding nth_safe_def using ** by (simp)
  qed
  moreover have 5: "\<forall>l|\<in>|the (locs_safe {||} m0 l2'). m1 $ l = m0 $ l"
  proof
    fix l' assume "l'|\<in>| the (locs_safe {||} m0 l2')"
    moreover have "l |\<in>| the (locs_safe {||} m0 l1)"
      by (metis "1" data.locs_safe_mlookup l_def locs_safe_def)
    moreover have 3: "locs_safe {||} m0 l2'= Some (the (locs_safe {||} m0 l2'))"  using s_data.locs_copy_memory_some
      by (metis assms(2,7) data.locs_safe_mlookup data.locs_safe_in_subs locs_safe_def option.sel s_data.copy_memory_def)
    ultimately have "l \<noteq> l'" using assms(5)
      by (smt (verit, ccfv_SIG) "3" assms(2,7) data.mlookup_locs_subs disjoint_iff_fnot_equal finsert_fsubset
          locs_safe_def mk_disjoint_finsert option.sel s_data.copy_memory_def s_data.locs_copy_memory_some
          s_data.locs_def)
    then show "m1 $ l' = m0 $ l'" unfolding nth_safe_def using ** by (simp)
  qed
  moreover have 6: "locations m0 is1 l1 = Some (the (locations m0 is1 l1))"
    by (simp add: l_def mlookup_locations_some)
  moreover have 7: "the (locations m0 is1 l1) |\<inter>| the (locs_safe {||} m0 l2') = {||}"
  proof -
    have "the (locations m0 is1 l1) |\<subseteq>| the (locs_calldata m0 l1)"
      by (metis "1" "6" s_data.locs_def s_data.locs_safe_locations)
    moreover have "the (locs_safe {||} m0 l2') |\<subseteq>| the (locs_safe {||} m0 l2)"
      by (metis "3" assms(2) option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
    ultimately show ?thesis using assms(5)
      by (metis (no_types, lifting) ext boolean_algebra_cancel.inf1 inf.order_iff inf_bot_right inf_commute
          s_data.locs_def)
  qed
  moreover have 8: "l |\<notin>| the (locs_safe {||} m0 l2')" using assms(5)
  proof -
    have "l |\<in>| the (locs_safe {||} m0 l1)"
      by (metis "1" data.locs_safe_mlookup l_def locs_safe_def)
    moreover have 3: "locs_safe {||} m0 l2'= Some (the (locs_safe {||} m0 l2'))"  using s_data.locs_copy_memory_some
      by (metis assms(2,7) data.locs_safe_mlookup data.locs_safe_in_subs locs_safe_def option.sel s_data.copy_memory_def)
    ultimately show ?thesis using assms(5)
      by (smt (verit, ccfv_SIG) "3" assms(2,7) data.mlookup_locs_subs disjoint_iff_fnot_equal finsert_fsubset
          ...
          s_data.locs_def)
  qed
  moreover have "locs_safe {||} m0 l2'= Some (the (locs_safe {||} m0 l2'))"  using s_data.locs_copy_memory_some
      by (metis assms(2,7) data.locs_safe_mlookup data.locs_safe_in_subs locs_safe_def option.sel s_data.copy_memory_def)
  ultimately obtain cd' where 9: "copy_memory_safe {||} m1 l1 = Some cd'"
    using s_data.update_some_obtains_copy[OF l_def 0 1 2 _ 4 5 _ _ _ 6 7 8] assms(4,6,7) 
    by (metis inf_bot_left s_data.copy_memory_def s_data.locs_def
        s_data.locs_safe_copy_memory_safe)
  moreover have "\<forall>l|\<in>|the (locs_safe {||} m0 l2). m1 $ l = m0 $ l"
  proof
    fix l' assume "l'|\<in>| the (locs_safe {||} m0 l2)"
    moreover have "l |\<in>| the (locs_safe {||} m0 l1)"
      by (metis "1" data.locs_safe_mlookup l_def locs_safe_def)
    moreover have 3: "locs_safe {||} m0 l2'= Some (the (locs_safe {||} m0 l2'))"  using s_data.locs_copy_memory_some
      by (metis assms(2,7) data.locs_safe_mlookup data.locs_safe_in_subs locs_safe_def option.sel s_data.copy_memory_def)
    ultimately have "l \<noteq> l'" using assms(5) by (metis disjoint_iff_fnot_equal s_data.locs_def)
    then show "m1 $ l' = m0 $ l'" unfolding nth_safe_def using ** by (simp)
  qed
  moreover from assms(6) have "copy_memory_safe {||} m0 l1 = Some cd1"
    by (simp add: s_data.copy_memory_def)
  moreover from assms(7) have "copy_memory_safe {||} m0 l2 = Some cd2"
    by (simp add: s_data.copy_memory_def)
  moreover have "check_calldata m0 (the (locs_safe {||} m0 l1))"
    by (metis assms(4) s_data.locs_def)
  ultimately show ?thesis using copy_memory_safe_lookup_update[OF l_def assms(2) 0 1 2 3 4 ]
    by (metis option.sel s_data.copy_memory_def)
qed

lemma copy_memory_mupdate:
  assumes "mvalue_update is1 (l0, v, m0) = Some m1"
      and "the (mlookup m0 is1 l0) |\<notin>| the (locs_calldata m0 l1)"
      and "copy_memory_calldata m0 l1 = Some cd0"
    shows "copy_memory_calldata m1 l1 = Some cd0"
proof -
  from assms(1) obtain l
    where l_def: "mlookup m0 is1 l0 = Some l"
    and *: "l < length m0"
    and **: "m1 = m0[l:=v]" using mvalue_update_obtain by metis
  moreover from assms(3) obtain L where L_def: "locs_calldata m0 l1 = Some L"
    by (metis s_data.copy_memory_def s_data.locs_copy_memory_some s_data.locs_def)
  moreover from assms(2) ** L_def l_def have "\<forall>l |\<in>| L. m1 $ l = m0 $ l" unfolding nth_safe_def
    apply (simp add: split:if_split_asm)
    by (metis nth_list_update_neq)
  ultimately show ?thesis using Memory.s_data.copy_memory_locs[OF assms(3)] by blast
qed
...
lemma check_locs_minit_1:
  assumes "Memory.minit cd m = (x1, x2)"
    shows "check_calldata x2 (the (locs_calldata x2 x1))"
proof -
  from assms(1) obtain L where "locs_calldata x2 x1 = Some L" using locs_locs_minit_1 by blast
  then show ?thesis using minit_check_calldata[OF assms(1)]
    by (metis bot_fset.rep_eq empty_iff option.sel s_data.locs_def)
qed
...
lemma check_locs_minit_2:
  assumes "Memory.minit cd m0 = (l1, m1)"
      and "locs_calldata m0 l0 = Some (the (locs_calldata m0 l0))"
      and "check_calldata m0 (the (locs_calldata m0 l0))"
    shows "check_calldata m1 (the (locs_calldata m1 l0))"
proof -
  from assms(1) have "prefix m0 m1"
    by (metis minit_sprefix snd_eqD sprefix_prefix)
  moreover have "fset (the (locs_calldata m0 l0)) \<subseteq> loc m0" using s_data.locs_subs2 assms(2) by blast
  ultimately have "check_calldata m1 (the (locs_calldata m0 l0))" using s_data.check_prefix[OF _ _ assms(3)] assms(2)
    by (metis s_data.locs_def s_data.locs_prefix)
  then show ?thesis
    by (metis \<open>prefix m0 m1\<close> assms(2) data.locs_prefix locs_calldata_def)
qed
...
lemma check_mupdate:
  assumes "mvalue_update is1 (l1, v, m0) = Some m1"
      ...
      and "check_calldata m0 (the (locs_calldata m0 l1))"
      and "locs_calldata m0 l2 = Some (the (locs_calldata m0 l2))"
      and "check_calldata m0 (the (locs_calldata m0 l2))"
      and "the (locs_calldata m0 l1) |\<inter>| the (locs_calldata m0 l2) = {||}"
    shows "check_calldata m1 (the (locs_calldata m1 l1))"
proof -
  from assms(1) obtain l
    where l_def: "mlookup m0 is1 l1 = Some l"
    and *: "l < length m0"
  ...
  then have 0: "m1 $ l = m0 $ l2'" by (simp add: assms(3))
  moreover have 2: "locs_safe {||} m0 l = Some (the (locs_safe {||} m0 l))"
    by (metis assms(4) l_def option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have 4: "\<forall>l|\<in>|the (locs_safe {||} m0 l1) |-| the (locs_safe {||} m0 l). m1 $ l = m0 $ l"
  proof
    fix l' assume "l' |\<in>|the (locs_safe {||} m0 l1) |-| (the (locs_safe {||} m0 l))"
    moreover have "l |\<notin>| the (locs_safe {||} m0 l1) |-| (the (locs_safe {||} m0 l))"
  ...
  proof
    fix l' assume "l'|\<in>| the (locs_safe {||} m0 l2)"
    moreover have "l |\<in>| the (locs_safe {||} m0 l1)"
      by (metis assms(4) l_def s_data.locs_def s_data.locs_mlookup)
    ultimately have "l \<noteq> l'" using assms(5,8)
      by (metis fempty_iff finterI s_data.locs_def)
    then show "m1 $ l' = m0 $ l'" unfolding nth_safe_def using ** by simp
  qed
  then have 5: "\<forall>l|\<in>|the (locs_safe {||} m0 l2'). m1 $ l = m0 $ l"
    by (metis assms(2,6) fsubsetD option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have 6: "locations m0 is1 l1 = Some (the (locations m0 is1 l1))"
    by (simp add: l_def mlookup_locations_some)
  moreover have "the (locations m0 is1 l1) |\<inter>| the (locs_safe {||} m0 l2) = {||}"
    by (smt (verit, best) "6" assms(4,8) finter_absorb1 finter_assoc finter_commute finter_fempty_left s_data.locs_def
        s_data.locs_locations)
  then have 7: "the (locations m0 is1 l1) |\<inter>| the (locs_safe {||} m0 l2') = {||}"
    by (smt (verit, best) assms(2,6) fsubset_fempty inf.cobounded1 inf.cobounded2 inf.order_iff inf_mono
        option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  moreover have "l |\<notin>| the (locs_safe {||} m0 l2)" using assms(5)
    by (metis assms(4,8) disjoint_iff_fnot_equal l_def s_data.locs_def s_data.locs_mlookup)
  then have 8: "l |\<notin>| the (locs_safe {||} m0 l2')"
    by (metis assms(2,6) finterD1 inf.absorb_iff2 option.sel s_data.locs_def
        s_data.locs_safe_mlookup_locs)
  moreover have 9: "locs_safe {||} m0 l1 = Some (the (locs_safe {||} m0 l1))"
    by (metis assms(4) s_data.locs_def)
  ultimately obtain L where L_def: "locs_safe {||} m1 l1 = Some L"
    using s_data.update_some_obtains_locs[OF l_def 0 _ 2 _ 4 5 _ 6 7 8 ] assms(5)
    by (metis assms(2,6) finter_fempty_left option.sel s_data.locs_def s_data.locs_safe_mlookup_locs)
  
  moreover from 9 have 9: "data.locs_safe {||} m0 l1 = Some (the (data.locs_safe {||} m0 l1))"
    by (metis locs_safe_def)
  moreover from 2 have 2: "data.locs_safe {||} m0 l = Some (the (locs_safe {||} m0 l))"
    by (metis locs_safe_def)
  moreover have 10: "data.locs_safe {||} m0 l2' = Some (the ( data.locs_safe {||} m0 l2'))"
    by (metis assms(2,6) data.locs_safe_mlookup_locs locs_calldata_def locs_safe_def option.sel
        s_data.locs_def)
  moreover from 4 have 4: "\<forall>l|\<in>|the (data.locs_safe {||} m0 l1) |-| the (locs_safe {||} m0 l). m1 $ l = m0 $ l"
    by (metis locs_safe_def)
  ...
  moreover have 11: "storage_data.check m0 (the (data.locs_safe {||} m0 l1))"
    by (metis assms(5) check_calldata_def data.locs_def locs_calldata_def)
  moreover have 12: "storage_data.check m0 (the (data.locs_safe {||} m0 l2'))"
  proof -
    from assms(7) have "storage_data.check m0 (the (locs_calldata m0 l2))"
      by (simp add: check_calldata_def)
    with 10 show ?thesis
      by (metis (no_types, lifting) assms(2,6) check_calldata_def data.locs_safe_mlookup_locs locs_calldata_def
          locs_safe_def option.sel s_data.check_subs s_data.locs_def)
  qed
  moreover from L_def have 13: "data.locs_safe {||} m1 l1 = Some L"
    by (metis locs_safe_def)
  moreover have "the (data.locs_safe {||} m0 l1) |-| the (locs_safe {||} m0 l) |\<inter>| the (data.locs_safe {||} m0 l2') =
    {||}"
  proof -
    from assms(8) have "the (data.locs_safe {||} m0 l1) |\<inter>| the (data.locs_safe {||} m0 l2) = {||}"
      by (simp add: locs_safe_def s_data.locs_def)
    then have "the (data.locs_safe {||} m0 l1) |\<inter>| the (data.locs_safe {||} m0 l2') = {||}"
      by (smt (verit, best) "10" assms(2,6) disjoint_iff_fnot_equal fsubsetD locs_safe_def locs_storage_safe_def
          s_data.locs_def storage_data.mlookup_locs_subs)
    then show ?thesis by blast
  qed
  ultimately have "storage_data.check m1 L" using data.check_update [OF l_def 0 9 2 10 4 5 11 12 13] by blast
  then show ?thesis
    by (simp add: "13" check_calldata_def data.locs_def locs_calldata_def)
qed

lemma check_mupdate2:
  assumes "mvalue_update is (l0, v, m0) = Some m1"
      and "locs_calldata m0 l1 = Some (the (locs_calldata m0 l1))"
      and "the (mlookup m0 is l0) |\<notin>| the (locs_calldata m0 l1)"
      and "check_calldata m0 (the (locs_calldata m0 l1))"
    shows "check_calldata m1 (the (locs_calldata m1 l1))"
proof -
  from assms(1) obtain l
  ...
  from assms(2) obtain L where L_def: "locs_calldata m0 l1 = Some L" by blast
  moreover from assms(3) l_def * ** have "\<forall>l'|\<in>|L. m1 $ l' = m0 $ l'" unfolding nth_safe_def apply (auto split:if_split)
    by (metis calculation nth_list_update_neq option.sel)
  ultimately have *: "locs_calldata m1 l1 = Some L" using assms(3) s_data.locs_same[of m0 l1 L] by blast
  then have "\<forall>l|\<in>|the (locs_calldata m0 l1). m0 $ l = m1 $ l" using L_def
    by (simp add: \<open>\<forall>l'|\<in>|L. m1 $ l' = m0 $ l'\<close>)
  then have "check_calldata m1 (the (locs_calldata m0 l1))" using s_data.check_check[OF assms(4,2)] by blast
...
lemma isValue_isArray_all:
  assumes "mdata.is_Value aa"
      ...
      and "m0 $ ya = Some aa"
      and "copy_memory_calldata m0 l0 = Some cd"
      and "sdata.is_Array (the (dlookup xs cd))"
  shows thesis
proof -
  from copy_memory_calldata_mlookup_obtains[OF _ assms(2), of "{||}" cd]
  obtain cd' where "copy_memory_safe {||} m0 ya = Some cd'" and "dlookup xs cd = Some cd'"
    using assms(4) unfolding s_data.copy_memory_def by blast
  with assms(1,3) have "sdata.is_Value cd'" by (auto simp add:case_memory_def split:option.split mdata.split_asm)
  then show ?thesis using assms
    by (simp add: \<open>dlookup xs cd = Some cd'\<close> sdata.distinct_disc(1))
qed
