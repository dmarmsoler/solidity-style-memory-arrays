...
lemma (in Contract) example:
  "wp (do {
        minit (sdata.Array [sdata.Array [sdata.Value (Bool False)]]) (STR ''x'');
        minit (sdata.Array [sdata.Array [sdata.Value (Bool False)]]) (STR ''y'');
      ...
      (pred_memory (STR ''x'') (\<lambda>cd. dlookup [Uint 0, Uint 0] cd = Some (sdata.Value (Bool True))))
      (K (K True))
      s"
  apply wp+
  apply (auto simp add: is_Array_minit dlookup.simps)
  apply wp+
  ...
  apply (erule locations_minit, simp add:dlookup.simps)
  apply (erule mlookup_locations_minit_2, simp add:dlookup.simps)
  apply (erule mlookup_some_minit, simp add:dlookup.simps)
  apply (erule mlookup_loc_minit, simp add:dlookup.simps)
  ...
  apply (erule mlookup_some_minit, simp add:dlookup.simps)
  apply (erule locations_minit,simp add: dlookup.simps)
  apply (erule mlookup_locations_minit_1,simp add: dlookup.simps)
  (*Aliasing*)
...
lemma (in Contract) example_short:
  ...
  (*Aliasing*)
  apply (drule_tac ?xs1.0 = "[Uint 0]" and ?l1.0=l in aliasing, simp, simp)
  apply (mc+, (auto simp add:dlookup.simps)[1])+
  apply (rule pred_some_copy_memory)
  apply mc+
  by (simp add:dlookup.simps)
